...
public:
    vector<int> luckyNumbers(vector<vector<int>>& matrix) {
        ...
        for (const auto& row : matrix) {
            row_maximum_of_minimums = max(*min_element(row.begin(), row.end()),
                                          row_maximum_of_minimums);
        }
        ...
        return row_maximum_of_minimums == col_minimum_of_maximums
                   ? vector<int>{col_minimum_of_maximums}
                   : vector<int>{};
    }
};