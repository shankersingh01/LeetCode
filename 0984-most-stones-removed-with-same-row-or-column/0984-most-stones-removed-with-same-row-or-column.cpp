...
private:
    ...
    int findRepresentative(int element, vector<int>& setRepresentatives) {
        ...
        return setRepresentatives[element] == element
                   ? element
                   : (setRepresentatives[element] = findRepresentative(
                          setRepresentatives[element], setRepresentatives));
    }

    void mergeComponents(int elementA, int elementB,
                         vector<int>& setRepresentatives) {
...
public:
    int removeStones(vector<vector<int>>& stonePositions) {
        ...
        for (const auto& stonePosition : stonePositions) {
            mergeComponents(stonePosition[0] + 1, stonePosition[1] + 10002,
                            setRepresentatives);
        }
