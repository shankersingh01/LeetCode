...
private:
    bool fillBobPath(int node, int parent, vector<int>& path,
        ...
        for (int neighbor : graph[node]) {
            if (neighbor != parent) {
                path.push_back(node);
                if (fillBobPath(neighbor, node, path, graph))
                    return true;
                path.pop_back();
            }
        }
    ...
    int getAliceMaxScore(int node, int parent, int currScore,
                         vector<int>& bobPath, vector<vector<int>>& graph,
        ...
        for (int neighbor : graph[node]) {
            if (neighbor != parent) {
                maxScore =
                    max(maxScore,
                        getAliceMaxScore(neighbor, node, currScore, bobPath,
                                         graph, timestamp + 1, amount));
            }
        }
        return maxScore;
    }
