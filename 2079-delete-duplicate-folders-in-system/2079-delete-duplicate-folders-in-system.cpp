...
// Time: O(NDWlogC + N^2 * W) where `N` is the number of folders, `W` is the
// maximum length of folder name,
//                                  `D` is the deepest folder depth, and `C` is
//                                  the maximum number of direct child folders.
// Space: O(N^2 * W)
struct Node {
    string name;
    map<string, Node*>
        next; // mapping from folder name to the corresponding child node.
...
class Solution {
    void addPath(
        Node* node,
        vector<string>& path) { // Given a path, add nodes to the folder tree.
    ...
    unordered_map<string, Node*> seen; // mapping from subfolder structure
                                       // string to the first occurrence node.
    string
    dedupe(Node* node) { // post-order traversal to dedupe. If we've seen the
                         // subfolder structure before, mark it as deleted.
        string subfolder;
        ...
        if (subfolder.size()) { // leaf nodes should be ignored
            if (seen.count(
                    subfolder)) { // if we've seen this subfolder structure
                                  // before, mark them as deleted.
